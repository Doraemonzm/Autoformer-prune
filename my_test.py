...
def main(args):
    ...
    new_model = model.structure_prune(pruned_config)

    new_model.to(device)

    new_model = torch.nn.parallel.DistributedDataParallel(new_model, device_ids=[args.gpu],
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, new_model, device, mode=args.mode, retrain_config=pruned_config)
