...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, super_cfg, img_size=224, patch_size=16, in_chans=3, num_classes=1000, qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.super_embed_dim = super_cfg['embed_dim'][0]
        # self.super_embed_dim = args.embed_dim
        self.super_mlp_ratio = super_cfg['mlp_ratio']
        self.super_layer_num = super_cfg['layer_num']
        self.super_num_heads = super_cfg['num_heads']
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate,  self.super_layer_num)]  # stochastic depth decay rule

        for i in range( self.super_layer_num):
            # print('The {}-th block:',i)
            # print('qkv_dim:',self.super_num_heads[i]*64)
            # print('super_num_head:', self.super_num_heads[i])
            self.blocks.append(TransformerEncoderLayer(dim=self.super_embed_dim, qkv_dim=self.super_num_heads[i]*64, num_heads=self.super_num_heads[i], mlp_ratio=self.super_mlp_ratio[i],
    ...
    def set_sample_config(self, config: dict):
        ...
        for i, blocks in enumerate(self.blocks):
            ...
            if i < self.sample_layer_num:
                # print('*********************************')
                # print('sample for the {}-th layer'.format(i))
                # print('emb_dim:',self.sample_embed_dim[i])
                # print('mlp ratio:', self.sample_mlp_ratio[i])
                # print('num heads:', self.sample_num_heads[i])
                # print('*********************************')
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, dim, qkv_dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, dropout=0., attn_drop=0.,
                 drop_path=0.,  pre_norm=True, scale=False,
        ...
        self.attn = AttentionSuper(
            dim, qkv_dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
    ...
    def set_sample_config(self, is_identity_layer, sample_embed_dim=None, sample_mlp_ratio=None, sample_num_heads=None, sample_dropout=None, sample_attn_dropout=None, sample_out_dim=None):
        ...
        self.attn.set_sample_config(sample_num_heads=self.sample_num_heads_this_layer, sample_in_embed_dim=self.sample_embed_dim)
...
#     a=Vision_TransformerSuper(super_cfg={'layer_num': 14,
#                      'embed_dim': [576, 576, 576, 576, 576, 576, 576, 576, 576, 576, 576, 576, 576, 576],
#                      'num_heads': [9, 10, 10, 10, 9, 10, 9, 10, 9, 10, 9, 9, 10, 10],
#                      'mlp_ratio': [3.5, 3.0, 3.5, 3.5, 3.0, 3.5, 3.0, 3.5, 3.0, 4.0, 3.0, 3.5, 4.0, 3.0]})
#     print(a)
