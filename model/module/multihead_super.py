...
class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, qkv_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., normalization = False, relative_position = False,
        ...
        self.proj_drop_ratio=proj_drop
        ...
        # print('qkv_dim here:',qkv_dim)
        self.qkv = qkv_super(super_embed_dim, 3 * qkv_dim, bias=qkv_bias)
        # else:
        ...
        self.proj = LinearSuper(qkv_dim, super_embed_dim)

        self.attn_drop = nn.Dropout(attn_drop)
        self.proj_drop = nn.Dropout(proj_drop)

    def set_sample_config(self, sample_num_heads=None, sample_in_embed_dim=None):
        ...
        #
        # else:
        self.sample_qk_embed_dim = sample_num_heads*64
        # print('sample num heads:',sample_num_heads)
        # print('sample qk embed dim:',self.sample_qk_embed_dim)
    ...
    def forward(self, x):
        B, N, C = x.shape
        # print('x.shape in attn is :', x.shape)
        # print('self.sample_num_heads is:', self.sample_num_heads)
        # print('x after qkv:', self.qkv(x).size())
        ...
        if self.relative_position:
            ...
            # print('q.size:', q.size())
            # print('N:',N)
            # print('sample num heads:', self.sample_num_heads)
            # print('r_p_k:', r_p_k.size())
            # print('B:',B)
            ...
            # attn = attn + (q.permute(2, 0, 1, 3).reshape(N, self.sample_num_heads * B, -1) @ r_p_k.transpose(2, 1)) \
            #     .transpose(1, 0).reshape(B, self.sample_num_heads, N, N)
