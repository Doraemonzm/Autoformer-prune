...
from supernet_engine_w_teacher import train_one_epoch, evaluate
from lib.samplers import RASampler
from lib import utils
from lib.config import cfg, update_config_from_file
# from model.supernet_transformer import Vision_TransformerSuper
from model.my_supernet import Vision_TransformerSuper
from easydict import EasyDict as edict
from prune_helper import prune_lowindex, new_prune

def get_args_parser():
    parser = argparse.ArgumentParser('AutoFormer training and evaluation script', add_help=False)
    parser.add_argument('--batch-size', default=64, type=int)
    parser.add_argument('--epochs', default=300, type=int)
    parser.add_argument('--epoch_intervel', default=None, type=int)
    # config file
    parser.add_argument('--cfg',help='experiment configure file name',required=True,type=str)
    parser.add_argument('--teacher_cfg', help='experiment configure file name', default=None, type=str)
    parser.add_argument('--teacher_resume', default=False, help='teacher resume from checkpoint')
    ...
    parser.add_argument('--prune', default='', help='prune from checkpoint')
...
def main(args):
    ...
    # update_config_from_file(args.pruned_cfg)
    ...
    print(f"Creating Origin SuperVisionTransformer")
    print(cfg)
    ...
    choices = {'num_heads': cfg.search_space.num_heads, 'mlp_ratio': cfg.search_space.mlp_ratio,
               'embed_dim': cfg.search_space.embed_dim , 'layer_num': cfg.search_space.layer_num}
    ...
    # args.epoch_intervel = None

    if args.prune:
        checkpoint = torch.load(args.prune, map_location='cpu')
        model= new_prune(model, checkpoint)
    ...
    if args.teacher_cfg:
        with open(args.teacher_cfg) as f:
            teacher_cfg = edict(yaml.safe_load(f))
        teacher_model = Vision_TransformerSuper(
        ...
        if args.teacher_resume:
            t_checkpoint = torch.load(args.teacher_resume, map_location='cpu')
            t_state_dict = teacher_model.state_dict()
            for k, v in t_checkpoint['model'].items():
                # strip `module.` prefix
                name = k[7:] if k.startswith('module') else k
                if name in t_state_dict.keys():
                    t_state_dict[name] = v
            teacher_model.load_state_dict(t_state_dict)
    ...
    #     model_without_ddp.load_state_dict(checkpoint['model'])
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            # print('checkpoint for optimizer is :')
            # new_state_dict = optimizer.state_dict()
            # for key, item in checkpoint['optimizer'].items():
            #     print('key:',key)
            #     print('item:', item.size())
            #     if key in new_state_dict.keys() and new_state_dict[key].size()==item.size():
    ...
    if args.mode == 'retrain' :
        retrain_config = cfg.supernet
        # if not args.pruned_cfg:
        #     retrain_config= cfg.supernet
        # else:
        #     retrain_config=exp_config.supernet
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            teach_loss=teacher_loss, teacher_cfg= teacher_cfg.supernet,
        ...
        if args.epoch_intervel and (epoch+1)% args.epoch_intervel==0:
            return
