...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, device, model,  choices, val_loader, test_loader, output_dir):
        ...
        self.memory = []
        self.vis_dict = {}
        self.keep_top_k = {self.select_num: [], 10: []}
    ...
    def save_checkpoint(self):
        ...
        checkpoint_path = os.path.join(self.output_dir, "search_checkpoint.pth.tar")
    ...
    def get_random_cand(self):
        ...
        depth = random.choice(self.choices['layer_num'])
        cand_tuple.append(depth)
        for dimension in dimensions:
            for i in range(depth):
                cand_tuple.append(random.choice(self.choices[dimension][i]))
    ...
    def search(self):
        print('population_num = {} select_num = {}'.format(self.population_num, self.select_num))
        ...
        self.update_top_k(
            self.candidates, k=10, key=lambda x: self.vis_dict[x]['acc'])

        print('top {} result'.format(
...
def get_args_parser():
    ...
    # parser.add_argument('--pruned_cfg', help='experiment configure file name', type=str)
...
def main(args):
    ...
    # if args.pruned_cfg:
    #     with open(args.pruned_cfg) as f:
    #         exp_config = edict(yaml.safe_load(f))
    #     print('Building Pruned SuperVisionTransformer')
    #     print(exp_config.supernet)
    #     choices = {'num_heads': exp_config.search_space.num_heads, 'mlp_ratio': exp_config.search_space.mlp_ratio,
    #                'embed_dim': exp_config.search_space.embed_dim, 'layer_num': exp_config.search_space.layer_num}
    #
    #     model = model.structure_prune(exp_config.supernet)
    ...
    t = time.time()
    searcher = EvolutionSearcher(args, device, model,  choices, data_loader_val, data_loader_test, args.output_dir)
